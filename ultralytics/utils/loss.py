...
try:
    # bbox_multi_iou、bbox_focal_multi_iou See the core code of the function ultralytics\utils\NewLoss\iouloss.py document 
    from .NewLoss.iouloss import bbox_multi_iou, bbox_focal_multi_iou
    # bbox_shape_iou, bbox_mpdiou, bbox_inner_multi_iou, bbox_piou, nwdiou, bbox_effciou, bbox_xiou See the core code of the function ultralytics\utils\NewLoss\ioulossone.py document 
    from .NewLoss.ioulossone import nwd, bbox_shape_iou, bbox_mpdiou, bbox_inner_multi_iou, bbox_piou, nwdiou, bbox_effciou, bbox_xiou

    # repulsionloss See the core code of the function ultralytics\utils\NewLoss\repulsionloss.py document 
    from .NewLoss.repulsionloss import repulsionloss

    # gwdloss See the core code of the function ultralytics\utils\NewLoss\gwdloss.py document ,  Detailed code in this file 
    from .NewLoss.gwdloss import gwdloss
    # kldloss See the core code of the function ultralytics\utils\NewLoss\kldloss.py document ,  Detailed code in this file 
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16):
        ...
        self.angle_dfl = DF_angle_Loss(reg_max) if reg_max > 1 else None
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        # origin iou
        try:
            useloss = ARGS_PA.loss # TAL Use default when executing CIoU,  So the default will be printed 
            #  Category IoU Loss function 
            if useloss == 'CIoU':
                iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True) # 🎈  correspond IoU The loss function comments can run the code 
            elif useloss == 'NWD':
                iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
                wasserstein = nwd(pred_bboxes[fg_mask], target_bboxes[fg_mask])
            elif useloss == 'GWDLoss': # GWDLoss Loss function 
                gwdloss = gwdloss(pred_bboxes[fg_mask], target_bboxes[fg_mask])
            elif useloss == 'KLDLoss': # KLDLoss Loss function 
                kldloss = kldloss(pred_bboxes[fg_mask], target_bboxes[fg_mask])
            elif useloss == 'RepulsionLoss': #  Small target cover loss function 
                iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
            elif useloss == 'MPDIoU':
                iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, GIoU=True)
            elif useloss == 'PIoU': # v1 v2 Both versions 
                iou = bbox_piou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, PIoU=True)
            elif useloss == 'PIoUv2': # v2 Version 
            ...
            elif useloss == 'WIoU': # v1 v2 v3 Both versions 
                ...
                iou = bbox_multi_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=True, UIoU=True, CIoU=True)
            elif useloss == 'GIoU':
            ...
            # print(' Default CIoU Loss function ')
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
        try:
            useloss = ARGS_PA.loss # TAL Use default when executing CIoU,  So the default will be printed 
            if useloss == 'WIoU':
        ...
        '''
            Loss Improve all kinds Loss：CIoU、DIoU、EIoU、GIoU、SIoU、WIoU
        ...
        '''
        '''
            Inner-IoU  Improve all kinds Loss  Can be used in conjunction ,  Already updated as follows 10+ kind 
        ...
        # loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
        '''
            FocalerIoU  Improve all kinds Loss  Can be used in conjunction ,  Already updated as follows 10+ kind 
        ...
        '''
        '''
            Focal Loss Improve all kinds Loss：FocalCIoU、FocalDIoU、FocalEIoU、FocalGIoU、FocalSIoU、FocalWIoU、Focal_PIoU、Focal_PIoUv2、
        '''
...
class RotatedBboxLoss(BboxLoss):
    ...
    def __init__(self, reg_max, ):
        ...
        # self.use_angle_dfl = DF_angle_Loss(reg_max) if reg_max > 1 else None
        try:
            from .script import load_script
            ARGS_PA = load_script()
            angleloss = ARGS_PA.angleloss
            print(angleloss)
            self.use_angle_dfl = angleloss #  When setting to True When ， Explanation 
        except Exception as e:
            # pass
            self.use_angle_dfl = False #  When setting to True When ， Explanation 
        self.angle_max = 90
    ...
    def forward(self, pred_dist, pred_angle, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        if self.use_angle_dfl:
            ...
            loss_angle_dfl = self.angle_dfl(pred_angle_pos, target_angle_pos) * weight
        ...
        return loss_iou, loss_dfl, loss_angle_dfl
...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        try:
            useloss = ARGS_PA.loss
            if useloss == 'RepulsionLoss':
        ...
        if assign_type == 'ATSS':
            ...
            print('ATSS Label allocation strategy ')
            ...
            print('DynamicATSS Label allocation strategy ')
        elif assign_type == 'v8':
            pass
        else:
        ...
        try:
            useloss = ARGS_PA.clsloss # TAL Use default when executing CIoU,  So the default will be printed 
            #  Category IoU Loss function 
        ...
        try:
            useloss = ARGS_PA.loss
            if useloss == 'RepulsionLoss':
                from ultralytics.utils.NewLoss.repulsionloss import repulsion_loss
                loss[3], loss[4] = repulsion_loss(
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        """Calculate and return the loss for the YOLO model."""
        # loss = torch.zeros(3, device=self.device)  # box, cls, dfl
        loss = torch.zeros(4, device=self.device)  # box, cls, dfl,dafl
        ...
        loss[1] *= self.hyp.cls  # cls gain
        loss[2] *= self.hyp.dfl  # dfl gain
        loss[3] *= self.hyp.dfl  # danglefl gain
