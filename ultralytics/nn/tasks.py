...
'''
Seeultralytics\nn\modules\CoreV8\BackboneThe following folder
'''
...
from ultralytics.nn.modules import RepVGGBlock, SimConv, RepBlock, Transpose, BiFusion
from ultralytics.nn.modules.CoreV8.Neck.QARepNeXt import QARepNeXt
...
from ultralytics.nn.modules import CPNDenseB, CSCDenseB, ReNLANDenseB, C3_DenseB, C2f_DenseB
from ultralytics.nn.modules import CPNMSB, CSCMSB, ReNLANMSB, C3_MSB, C2f_MSB
# from ultralytics.nn.modules import CPNLSK, CSCLSK, ReNLANLSK, C3_LSK, C2f_LSK

from ultralytics.nn.modules import CPNGhostblockv2, CSCGhostblockv2, ReNLANGhostblockv2, C3_Ghostblockv2, C2f_Ghostblockv2
...
from ultralytics.nn.modules.CoreV8.Backbone.resnet import ResNet50vd,ResNet50vd_dcn,ResNet101vd,PPConvBlock,CoordConv,Res2net50

# Another way
from ultralytics.nn.modules.CoreV8.Impove.Dysample import DySample
from ultralytics.nn.modules.CoreV8.Backbone.shiftconv import ReparamLKBlock
from ultralytics.nn.modules.CoreV8.Backbone.efficientnet import stems, MBConvBlock
from ultralytics.nn.modules.CoreV8.Backbone.efficientnetv2 import FusedMBConv, MBConv
from ultralytics.nn.modules.CoreV8.Backbone.swin import CSwinTR, SwinTRX, SwinTRY, SwinTRZ, SwinV2TRX, SwinV2TRY, SwinV2TRZ, CSwinTRv2
# from ultralytics.nn.modules.CoreV8.ALL.msfm import MSFM, LAE
# from ultralytics.nn.modules.CoreV8.ALL.RFAC import RFAConv
from ultralytics.nn.modules.CoreV8.Attention.ema import EMA
from ultralytics.nn.modules.CoreV8.Conv.dodconv import DODConv
from ultralytics.nn.modules.CoreV8.Conv.pwconv import FasterNeXt
...
'''
    AttentionImproved point update
    SimAM,
    GAMAttention,
    CBAM,
    SKAttention,
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if m in [CPNMSB, CSCMSB, C3_MSB, C2f_MSB]:
                args.insert(2, n)  # number of repeats
                n = 1
        # elif m in [CPNLSK, CSCLSK, ReNLANLSK, C3_LSK, C2f_LSK]:
        #     c1, c2 = ch[f], args[0]
        #     if c2 != nc:  # if not output
        #         c2 = make_divisible(min(c2, max_channels) * width, 8)
        #     args = [c1, c2, *args[1:]]
        #     if m in [CPNLSK, CSCLSK, C3_LSK, C2f_LSK]:
        #         args.insert(2, n)  # number of repeats
        #         n = 1
        ...
        # Attention Attention mechanism summary
        ...
        #         args.insert(2, n)  # number of repeats
        #         n = 1
        elif m in [EMA, FasterNeXt]:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if not output
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            if m in [EMA, FasterNeXt]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is DODConv:
            c1, c2 = ch[f], args[0]
