...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias):
    ...
    if has_large_impl and in_channels == out_channels and out_channels == groups and use_large_impl and stride == 1 and padding == kernel_size // 2 and dilation == 1:
        ...
        from depthwise_conv2d_implicit_gemm import DepthWiseConv2dImplicitGEMM #  No need to pay attention to this 
...
class CPNRepLKBlock(nn.Module):
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5):
        ...
        self.m = nn.Sequential(*(RepLKBlock(self.c, self.c) for _ in range(n)))
    def forward(self, x):
        a, b = self.cv1(x).chunk(2, 1)
        return self.cv2(torch.cat((self.m(a), b), 1))

class C3_RepLKBlock(nn.Module):
    # C3_RepLKBlock Bottleneck with 3 convolutions
