...
class QARepVGGB(RepVGGB):
    ...
    def get_equivalent_kernel_bias(self):
        ...
        bias = bias3x3 # @CSDN Mango juice has no mango 
...
class QARep(nn.Module):
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5):
        ...
        self.m = nn.Sequential(*(QARepVGGB(self.c, self.c) for _ in range(n)))
    def forward(self, x):
        a, b = self.cv1(x).chunk(2, 1)
        return self.cv2(torch.cat((self.m(a), b), 1))

class C3_QARep(nn.Module):
    # C3_QARep Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(QARepVGGB(c_, c_) for _ in range(n)))
