...
def add_coord(x):
    ins_feat = x  #  Current instance features tensor
    #  Generate -1 arrive 1 Linear value 
    x_range = torch.linspace(-1, 1, ins_feat.shape[-1], device=ins_feat.device)
    y_range = torch.linspace(-1, 1, ins_feat.shape[-2], device=ins_feat.device)
    y, x = torch.meshgrid(y_range, x_range, indexing='ij')  #  Generate a two -dimensional coordinate grid
    y = y.expand([ins_feat.shape[0], 1, -1, -1])  #  Expand to harmony ins_feat Same dimension 
    x = x.expand([ins_feat.shape[0], 1, -1, -1])
    coord_feat = torch.cat([x, y], 1)  #  Location 
    ins_feat = torch.cat([ins_feat, coord_feat], 1)  # concatnate Together as the next convolution input 
