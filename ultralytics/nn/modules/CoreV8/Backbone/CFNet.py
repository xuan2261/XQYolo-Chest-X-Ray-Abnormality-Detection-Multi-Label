...
class FocalNeXtF(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(FocalNextBlock(self.c) for _ in range(n)))
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
################# 
class CPNFocalNeXt(nn.Module):
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)  
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c2, 1) 
        self.m = nn.Sequential(*(FocalNeXtF(self.c, self.c) for _ in range(n)))
    def forward(self, x):
        a, b = self.cv1(x).chunk(2, 1)
        return self.cv2(torch.cat((self.m(a), b), 1))

class C3_FocalNeXt(nn.Module):
    # C3_FocalNeXt Bottleneck with 3 convolutions
    # https://github.com/iscyy/ultralyticsPro
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(FocalNeXtF(c_, c_) for _ in range(n)))
...
class ReNLANFocalNeXt(nn.Module):
    # ReNLANFocalNeXt Block
...
'''
YOLOv8 Improved project :  Details ,  Can add a blogger's contact information  QQ: 2434798737 ⭐
