    ...
    for result in results:
        for box in result.boxes:
            ...
            if class_id < 20: # chỉnh class_id < 14: để giới hạn không hiển thị lớp Finding và No finding
                ...
                draw.rectangle([(x_min, y_min - text_height), (x_min + text_width, y_min)], fill=color)
    ...
    if uploaded_file is not None:
        ...
        if st.button("Detect Abnormalities"):
            with st.spinner("Model is making predictions..."):
                ...
                output_image, info_list, color_list = draw_bounding_boxes(resized_image.copy(), results)
